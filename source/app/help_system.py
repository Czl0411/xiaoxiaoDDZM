...
HELP_ENTRIES: tuple[HelpEntry, ...] = (
    ...
    HelpEntry("委托", "/需求：完整内容", (), "AI解析需求并生成预览", "/需求：找1个人完成指定任务，每人200功德", (), "user", "main", True, 10, 10),
    HelpEntry("委托", "/服务：完整内容", (), "AI解析可出售的服务商品", "/服务：陪聊一小时，每次100功德，库存3份，上架3天", (), "user", "main", True, 20, 10),
    HelpEntry("委托", "/确认发布｜/取消发布", (), "确认或放弃当前草稿", "/确认发布", (), "user", "main", True, 30, 20),
    ...
    HelpEntry("委托", "/我的委托｜/我的需求｜/我的服务｜/我的订单", (), "查看本人发布和交易", "/我的订单", (), "user", "bounty", True, 70, 40),
    ...
    HelpEntry("委托", "/关闭需求D编号｜/下架服务S编号", (), "停止招募或出售", "/关闭需求D0001", (), "user", "bounty", True, 110, 70),
    HelpEntry("委托", "/删除需求D编号｜/删除服务S编号", (), "删除已结束且无未完成订单的发布", "/删除服务S0001", (), "user", "bounty", True, 115, 70),
    HelpEntry("委托", "/补充库存S编号*数量｜/续期服务S编号*天数", (), "管理有限库存和上架期限", "/补充库存S0001*5", (), "user", "bounty", True, 120, 70),
...
def _bounty_detail(features: dict[str, Any]) -> tuple[str, ...]:
    ...
    third = "\n".join(
        (
            ...
            "服务支持同一买家重复购买；不限人数即不限库存",
            "服务抽成从卖家收入扣除，买家不额外支付手续费",
